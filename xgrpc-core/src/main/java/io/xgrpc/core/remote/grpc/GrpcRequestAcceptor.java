    ...
    public void request(Payload grpcRequest, StreamObserver<Payload> responseObserver) {
        ...
        if (!ApplicationUtils.isStarted()) {
            Payload payloadResponse = GrpcUtils.convert(
                    ErrorResponse.build(XgrpcException.INVALID_SERVER_STATUS, "Server is starting,please try later."));
        ...
        if (requestHandler == null) {
            ...
            Payload payloadResponse = GrpcUtils
                    .convert(ErrorResponse.build(XgrpcException.NO_HANDLER, "RequestHandler Not Found"));
            traceIfNecessary(payloadResponse, false);
            responseObserver.onNext(payloadResponse);
            responseObserver.onCompleted();
            return;
        ...
        if (!requestValid) {
            ...
            Payload payloadResponse = GrpcUtils
                    .convert(ErrorResponse.build(XgrpcException.UN_REGISTER, "Connection is unregistered."));
            traceIfNecessary(payloadResponse, false);
            responseObserver.onNext(payloadResponse);
            responseObserver.onCompleted();
            return;
        ...
        try {
            ...
            Payload payloadResponse = GrpcUtils.convert(ErrorResponse.build(XgrpcException.BAD_GATEWAY, e.getMessage()));
            traceIfNecessary(payloadResponse, false);
            responseObserver.onNext(payloadResponse);
            responseObserver.onCompleted();
            return;
        ...
        if (parseObj == null) {
            ...
            Payload payloadResponse = GrpcUtils
                    .convert(ErrorResponse.build(XgrpcException.BAD_GATEWAY, "Invalid request"));
            traceIfNecessary(payloadResponse, false);
            responseObserver.onNext(payloadResponse);
            responseObserver.onCompleted();
            return;
