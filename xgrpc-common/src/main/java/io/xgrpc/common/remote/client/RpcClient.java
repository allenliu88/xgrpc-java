...
package io.xgrpc.common.remote.client;

import static io.xgrpc.api.exception.XgrpcException.SERVER_ERROR;
...
import io.xgrpc.api.remote.request.ClientDetectionRequest;
...
import io.xgrpc.api.remote.response.ClientDetectionResponse;
...
import io.xgrpc.common.utils.LoggerUtils;
import io.xgrpc.common.utils.NumberUtils;
...
public abstract class RpcClient implements Closeable {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger("io.xgrpc.common.remote.client");
     ...
     * Start this client.
     */
    public final void start() throws XgrpcException {
        ...
        clientEventExecutor = new ScheduledThreadPoolExecutor(2, r -> {
            ...
            t.setName("io.xgrpc.client.remote.worker");
            t.setDaemon(true);
            return t;
        });
     ...
     */
    public Response request(Request request) throws XgrpcException {
    ...
    public Response request(Request request, long timeoutMills) throws XgrpcException {
        ...
        while (retryTimes < RETRY_TIMES && System.currentTimeMillis() < timeoutMills + start) {
            ...
            try {
                if (this.currentConnection == null || !isRunning()) {
                    waitReconnect = true;
                    throw new XgrpcException(XgrpcException.CLIENT_DISCONNECT,
                            "Client not connected, current status:" + rpcClientStatus.get());
                }
                response = this.currentConnection.request(request, timeoutMills);
                if (response == null) {
                    throw new XgrpcException(SERVER_ERROR, "Unknown Exception.");
                }
                if (response instanceof ErrorResponse) {
                    if (response.getErrorCode() == XgrpcException.UN_REGISTER) {
                        synchronized (this) {
                            waitReconnect = true;
                    ...
                    throw new XgrpcException(response.getErrorCode(), response.getMessage());
                }
                // return response.
                lastActiveTimeStamp = System.currentTimeMillis();
                return response;
        ...
        if (exceptionThrow != null) {
            throw (exceptionThrow instanceof XgrpcException) ? (XgrpcException) exceptionThrow
                    : new XgrpcException(SERVER_ERROR, exceptionThrow);
        } else {
            throw new XgrpcException(SERVER_ERROR, "Request fail, unknown Error");
        }
    }
     ...
     * @param request request.
     */
    public void asyncRequest(Request request, RequestCallBack callback) throws XgrpcException {
        int retryTimes = 0;
        ...
        while (retryTimes < RETRY_TIMES && System.currentTimeMillis() < start + callback.getTimeout()) {
            ...
            try {
                if (this.currentConnection == null || !isRunning()) {
                    waitReconnect = true;
                    throw new XgrpcException(XgrpcException.CLIENT_INVALID_PARAM, "Client not connected.");
                }
        ...
        if (exceptionToThrow != null) {
            throw (exceptionToThrow instanceof XgrpcException) ? (XgrpcException) exceptionToThrow
                    : new XgrpcException(SERVER_ERROR, exceptionToThrow);
        } else {
            throw new XgrpcException(SERVER_ERROR, "AsyncRequest fail, unknown error");
        }
    }
    ...
    public RequestFuture requestFuture(Request request) throws XgrpcException {
        ...
        if (exceptionToThrow != null) {
            ...
            throw new XgrpcException(SERVER_ERROR, "Request future fail, unknown error");
        }
