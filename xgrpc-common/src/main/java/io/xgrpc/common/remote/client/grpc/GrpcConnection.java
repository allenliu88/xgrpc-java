...
import io.xgrpc.api.remote.RpcScheduledExecutor;
...
public class GrpcConnection extends Connection {
    ...
    @Override
    public Response request(Request request, long timeouts) throws XgrpcException {
    ...
    @Override
    public void asyncRequest(Request request, final RequestCallBack requestCallBack) throws XgrpcException {
        Payload grpcRequest = GrpcUtils.convert(request);
        ListenableFuture<Payload> requestFuture = grpcFutureServiceStub.request(grpcRequest);
        ...
        Futures.addCallback(requestFuture, new FutureCallback<Payload>() {
            ...
            public void onSuccess(@Nullable Payload grpcResponse) {
                ...
                if (response != null) {
                    if (response instanceof ErrorResponse) {
                        requestCallBack.onException(new XgrpcException(response.getErrorCode(), response.getMessage()));
                    } else {
                        requestCallBack.onResponse(response);
                    }
                } else {
                    requestCallBack.onException(new XgrpcException(ResponseCode.FAIL.getCode(), "response is null"));
                }
            }
